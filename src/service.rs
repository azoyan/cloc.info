...
pub fn create_general_router(provider: Arc<RwLock<RepositoryProvider>>) -> Router<(), Body> {
...
async fn raw_content(
    ...
    let (response, _branch_id) = match result {
        ...
        Err(repository_provider::Error::NotFound { url: _ }) => (
            Response::builder()
                .status(StatusCode::SWITCHING_PROTOCOLS)
                .header("Upgrade", "websocket")
                .header("Connection", "Upgrade")
        ...
        Err(e) => {
            if let repository_provider::Error::InProgress { url } = e {
                tracing::warn!("Repository {url} downloading already in progress");
                (
                    Response::builder()
...
async fn handler_with_branch(
    ...
    let branch = branch_name.trim_start_matches('/');
